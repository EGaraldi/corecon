...
def _LoadDataIntoDictionaryPy(filepath, dictionary, parent_field):
    ...
    if "limit_confidence_level" in local_var_dict:
        limit_confidence_level        = np.array(local_var_dict["limit_confidence_level"           ], dtype=float) ; del local_var_dict["limit_confidence_level"           ]
    else:
        limit_confidence_level        = np.array([np.nan], dtype=float)
    ...
    limit_confidence_level = _expand_field(limit_confidence_level, values.shape)
    ...
    if ndim == 0:
        ...
        for arr in [values, err_up, err_down, lower_lim, upper_lim, limit_confidence_level]:
    ...
    if (ndim > 1) and (data_structure == 'grid'):
        ...
        limit_confidence_level = limit_confidence_level.flatten() 
    ...
    limit_confidence_level = limit_confidence_level[~w]
    for k in extra_data.keys():
    ...
    dictionary[dictionary_tag] = \
            DataEntry(
                      ...
                      upper_lim              = upper_lim,
                      lower_lim              = lower_lim,
                      limit_confidence_level = limit_confidence_level,
