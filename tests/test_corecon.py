def test_get_fields():
    ...
    fields = ['HII_fraction', 'HeIII_fraction', 'Lya_flux_power_spectrum', 'mean_free_path', 'effective_optical_depth_HI_Lya',
              'effective_optical_depth_HeII_Lya', 'HeII_to_HI_column_density_ratio',a'quasar_luminosity_function',
    ...
    for f in fields:
        assert f in crc_fields, "Problem detected in CoReCon.get_fields"
...
def test_get_field_synonyms():
    ...
    for s in synonyms:
        assert s in crc_syn, "Problem detected in CoReCon.get_field_synonym"
